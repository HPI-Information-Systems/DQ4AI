...
QUALITY_NAME_MAP = {
    'adj_mut_info': 'Adjusted Mutual Information Score',
...
ORIGINAL_DATASET_AMI = {
    'bank.csv': {
        'Agglomerative': 0.06843965855921104,
        'Autoencoder': 0,
        'Gaussian Mixture': 0.0984361407662388,
        'OPTICS': 0.0054526146653422845,
        'k-Prototypes': 0.029920100299948054
    },
    'covtype.csv': {
    ...
    'letter.arff': {
        'Agglomerative': 0.40194653859220414,
        'Autoencoder': 0.05301928967610593,
        'Gaussian Mixture': 0.4436551716570202,
        'OPTICS': 0.18824366400086745,
        'k-Means': 0.35153971837917486
    }
}
    ...
    for i, (algo, res_df) in enumerate(algorithm_to_metrics.items()):
        for c in res_df.columns:
            ...
            if metric_figures.get(c) is None:
                ...
                ax.set_ylabel(f'Mean {QUALITY_NAME_MAP[c]}', size=36)
            ...
            ax.plot(res_df.index.to_list(), res_df[c], color=ALGO_COLOR_MAP[algo], marker=ALGO_MARKER_MAP[algo], label=algo, ms=15)
            # special handling of ConsistentRepresentationPolluter as it requires sorting by and annotation of pollution level
            if polluter_name == 'ConsistentRepresentationPolluter':
                texts.extend([
